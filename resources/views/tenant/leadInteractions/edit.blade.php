...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.leadInteraction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("tenant.lead-interactions.update", [$leadInteraction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead') ? 'is-invalid' : '' }}" name="lead_id" id="lead_id" required>
                    @foreach($leads as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lead_id') ? old('lead_id') : $leadInteraction->lead->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('lead'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('communication_channel') ? 'is-invalid' : '' }}" name="communication_channel" id="communication_channel">
                    ...
                    @foreach(App\Models\LeadInteraction::COMMUNICATION_CHANNEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('communication_channel', $leadInteraction->communication_channel) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $leadInteraction->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $leadInteraction->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
