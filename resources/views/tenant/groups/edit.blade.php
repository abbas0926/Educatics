...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.groups.update", [$group->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('promotion') ? 'is-invalid' : '' }}" name="promotion_id" id="promotion_id" required>
                    @foreach($promotions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('promotion_id') ? old('promotion_id') : $group->promotion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
