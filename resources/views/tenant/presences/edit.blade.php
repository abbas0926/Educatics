...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.lessons.update", [$lesson->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group_id" id="group_id" required>
                    @foreach($groups as $id => $entry)
                        <option value="{{ $id }}" {{ (old('group_id') ? old('group_id') : $lesson->group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('group'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('teacher') ? 'is-invalid' : '' }}" name="teacher_id" id="teacher_id" required>
                    @foreach($teachers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('teacher_id') ? old('teacher_id') : $lesson->teacher->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_at') ? 'is-invalid' : '' }}" type="text" name="start_at" id="start_at" value="{{ old('start_at', $lesson->start_at) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('ends_at') ? 'is-invalid' : '' }}" type="text" name="ends_at" id="ends_at" value="{{ old('ends_at', $lesson->ends_at) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('classroom') ? 'is-invalid' : '' }}" name="classroom_id" id="classroom_id" required>
                    @foreach($classrooms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('classroom_id') ? old('classroom_id') : $lesson->classroom->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('done') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="done" id="done" value="1" {{ $lesson->done || old('done', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('presence_students') ? 'is-invalid' : '' }}" name="presence_students[]" id="presence_students" multiple>
                    @foreach($presence_students as $id => $presence_student)
                        <option value="{{ $id }}" {{ (in_array($id, old('presence_students', [])) || $lesson->presence_students->contains($id)) ? 'selected' : '' }}>{{ $presence_student }}</option>
                    @endforeach
                </select>
