...
@can('testimonial_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('tenant.testimonials.create') }}">
                        ...
                        <tr data-entry-id="{{ $testimonial->id }}">
                            ...
                            <td>
                                @can('testimonial_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('tenant.testimonials.show', $testimonial->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('testimonial_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('tenant.testimonials.edit', $testimonial->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('testimonial_delete')
                                    <form action="{{ route('tenant.testimonials.destroy', $testimonial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('testimonial_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('tenant.testimonials.massDestroy') }}",
