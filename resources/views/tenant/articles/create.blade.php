...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.articles.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label for="subtitle">{{ trans('cruds.article.fields.subtitle') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.article.fields.subtitle_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="gallery">{{ trans('cruds.article.fields.gallery') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.article.fields.gallery_helper') }}</span>
            </div>
            <div class="form-group">
                <label>{{ trans('cruds.article.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Article::STATUS_SELECT as $key => $label)
            ...
            <div class="form-group">
                <label for="author">{{ trans('cruds.article.fields.author') }}</label>
                <input class="form-control {{ $errors->has('author') ? 'is-invalid' : '' }}" type="text" name="author" id="author" value="{{ old('author', '') }}">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('tenant.articles.storeCKEditorImages') }}', true);
...
<script>
    ...
    url: '{{ route('tenant.articles.storeMedia') }}',
    maxFilesize: 4, // MB
...
@if(isset($article) && $article->gallery)
      var files = {!! json_encode($article->gallery) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview)
