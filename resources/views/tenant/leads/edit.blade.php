...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.leads.update", [$lead->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="first_name">{{ trans('cruds.lead.fields.first_name') }}</label>
                <input class="form-control {{ $errors->has('first_name') ? 'is-invalid' : '' }}" type="text" name="first_name" id="first_name" value="{{ old('first_name', $lead->first_name) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Lead::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $lead->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('source') ? 'is-invalid' : '' }}" name="source" id="source">
                    ...
                    @foreach(App\Models\Lead::SOURCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('source', $lead->source) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" type="text" name="notes" id="notes" value="{{ old('notes', $lead->notes) }}">
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('formations') ? 'is-invalid' : '' }}" name="formations[]" id="formations" multiple>
                    @foreach($formations as $id => $formation)
                        <option value="{{ $id }}" {{ (in_array($id, old('formations', [])) || $lead->formations->contains($id)) ? 'selected' : '' }}>{{ $formation }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('events') ? 'is-invalid' : '' }}" name="events[]" id="events" multiple>
                    @foreach($events as $id => $event)
                        <option value="{{ $id }}" {{ (in_array($id, old('events', [])) || $lead->events->contains($id)) ? 'selected' : '' }}>{{ $event }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('marketing_campaign') ? 'is-invalid' : '' }}" name="marketing_campaign_id" id="marketing_campaign_id">
                    @foreach($marketing_campaigns as $id => $entry)
                        <option value="{{ $id }}" {{ (old('marketing_campaign_id') ? old('marketing_campaign_id') : $lead->marketing_campaign->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
