...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.marketingCampaign.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("tenant.marketing-campaigns.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leads') ? 'is-invalid' : '' }}" name="leads[]" id="leads" multiple>
                    @foreach($leads as $id => $lead)
                        <option value="{{ $id }}" {{ in_array($id, old('leads', [])) ? 'selected' : '' }}>{{ $lead }}</option>
                    @endforeach
                </select>
                @if($errors->has('leads'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('expenses') ? 'is-invalid' : '' }}" name="expenses[]" id="expenses" multiple>
                    @foreach($expenses as $id => $expense)
                        <option value="{{ $id }}" {{ in_array($id, old('expenses', [])) ? 'selected' : '' }}>{{ $expense }}</option>
                    @endforeach
                </select>
