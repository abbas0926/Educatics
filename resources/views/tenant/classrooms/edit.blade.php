...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.classrooms.update", [$classroom->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.classroom.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $classroom->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('capacity') ? 'is-invalid' : '' }}" type="number" name="capacity" id="capacity" value="{{ old('capacity', $classroom->capacity) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('equipments') ? 'is-invalid' : '' }}" name="equipments" id="equipments">{!! old('equipments', $classroom->equipments) !!}</textarea>
