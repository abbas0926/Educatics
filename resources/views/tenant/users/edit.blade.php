...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.users.update", [$user->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <label for="attachements">{{ trans('cruds.user.fields.attachements') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.attachements_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="salary">{{ trans('cruds.user.fields.salary') }}</label>
                <input class="form-control {{ $errors->has('salary') ? 'is-invalid' : '' }}" type="number" name="salary" id="salary" value="{{ old('salary', $user->salary) }}" step="0.01">
...
<script>
    ...
    url: '{{ route('tenant.users.storeMedia') }}',
    maxFilesize: 5, // MB
...
Dropzone.options.attachementsDropzone = {
    ...
    init: function () {
@if(isset($user) && $user->attachements)
          var files =
            {!! json_encode($user->attachements) !!}
              for (var i in files) {
              var file = files[i]
              this.options.addedfile.call(this, file)
              file.previewElement.classList.add('dz-complete')
