...
@can('task_status_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('tenant.task-statuses.create') }}">
                        ...
                        <tr data-entry-id="{{ $taskStatus->id }}">
                            ...
                            <td>
                                @can('task_status_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('tenant.task-statuses.show', $taskStatus->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('task_status_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('tenant.task-statuses.edit', $taskStatus->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('task_status_delete')
                                    <form action="{{ route('tenant.task-statuses.destroy', $taskStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('task_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('tenant.task-statuses.massDestroy') }}",
